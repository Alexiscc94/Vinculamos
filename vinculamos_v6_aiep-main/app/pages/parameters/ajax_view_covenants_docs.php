<?php
	...
	$datasCovenant = getCovenant($covenant);
	  			...
	  			<td><?php echo $datas[$i]['descripcion'];?></td>
					<td width="130">
						<?php
	  					if(canReadParameters()) {
								if($datas[$i]['archivo'] == "") {
									echo "<span class='small label label-danger'>Archivo no encontrado. Por favor volver a cargar.</span>&nbsp;";
								} else {?>
								<a href="<?php echo $datas[$i]['archivo'];?>" class='btn btn-orange' title='Ver'
						...
						<?php
	  					if(canUpdateParameters()) { ?>
								<a href="#" class='btn btn-orange' title='Editar'
						...
						<?php
	  					if(canDeleteParameters()) { ?>
								<a href="#" class='btn btn-default' title='Eliminar'
									data-id='<?php echo noeliaEncode($datas[$i]['id']);?>'
									data-nombre='<?php echo $datas[$i]['nombre'];?>'
									data-nombre_convenio='<?php echo $datasCovenant[0]['nombre'];?>'
									data-usuario='<?php echo $_SESSION["nombre_usuario"];?>'
