...
<div class="wrapper">
	<?php
		...
		include_once("modal_add_department.php");
		include_once("modal_delete_department.php");
    	...
    	<section class="content-header">
      		<h1>
			   Ámbitos de contribución. 
        		<small>todos los ámbitos de contribución.</small>
      		</h1>
      		<ol class="breadcrumb">
        		<li><i class="fa fa-home"></i> Inicio</li>
        		<li>Ámbitos de contribución. </li>
        		<li class="active">Ver ámbitos de contribución. </li>
      		</ol>
    	</section>
				...
				<div class="col-xs-12">
					<div class="box">
						<div class="box-header">
    						<h3 class="box-title">Listado de los ámbitos de contribución. </h3>

    						<?php
    							if(canCreateParameters()) {?>
    								<!-- <div class="btn-group pull-right">
										<button id="exportButton"  name="exportButton" class="btn btn-orange pull-right" data-toggle="modal" data-target="#addDepartment">
											<span class="fa fa-plus"></span> Agregar Ámbito
										</button>
									</div> -->
							<?php
								} ?>
...
<script>
	...
	$("#add_department").submit(function( event ) {
		$('#add_department').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			url: "./ajax_add_department.php",
			data: parametros,
	...
	$("#delete_department").submit(function( event ) {
		$('#delete_department').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			...
			success: function(datos) {
				$('#delete_department').attr("disabled", false);
	...
	$('#addCarrer').on('show.bs.modal', function (event) {
		...
		var id = button.data('id');
		var modal = $(this);
		modal.find('.modal-body #vg_id_facultad').val(id);
		modal.find('.modal-body #vg_nombre').val("");
		modal.find('.modal-body #vg_descripcion').val("");
