...
<div class="wrapper">
	<?php
		...
		include_once("modal_add_covenant.php");
		include_once("modal_delete_covenant.php");
	...
	<div class="content-wrapper">
    	...
    	<section class="content-header">
      		<h1>
        		Convenios
        		<small>todos los convenios</small>
      		</h1>
      		<ol class="breadcrumb">
        		<li><i class="fa fa-home"></i> Inicio</li>
        		<li>Convenios</li>
        		<li class="active">Ver Convenios</li>
      		</ol>
    	</section>
				...
				<div class="col-xs-12">
					<div class="box">
						<div class="box-header">
    						<h3 class="box-title">Listado de convenios</h3>

    						<?php
    							if(canCreateParameters()) {?>
    								<div class="btn-group pull-right">
										<button id="exportButton" name="exportButton" class="btn btn-orange pull-right" data-toggle="modal" data-target="#addCovenant">
											<span class="fa fa-plus"></span> Agregar Convenio
										</button>
									</div>
							<?php
								} ?>
...
<script>
	...
	$("#add_covenant").submit(function( event ) {
		$('#add_covenant').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			url: "./ajax_add_covenant.php",
			data: parametros,
			beforeSend: function(objeto) {
				$("#resultados_modal_agregar").html("Cargando...");
			},
			success: function(datos) {
				$('#add_covenant').attr("disabled", false);
	...
	$('#addCovenant').on('show.bs.modal', function (event) {
		var button = $(event.relatedTarget); // Button that triggered the modal
		var modal = $(this);
		modal.find('.modal-body #vg_nombre').val("");
		modal.find('.modal-body #vg_descripcion').val("");

		modal.find('.modal-body #vg_direccion').val("");
		modal.find('.modal-body #vg_nombre_contacto').val("");
		modal.find('.modal-body #vg_telefono').val("");
		modal.find('.modal-body #vg_correo_electronico').val("");
		modal.find('.modal-body #vg_fecha_suscripcion').val("");
		modal.find('.modal-body #vg_observaciones').val("");

		$("#resultados_modal_agregar").html("");
	})

	$("#delete_covenant").submit(function( event ) {
		$('#delete_covenant').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			url: "./ajax_delete_covenant.php",
			data: parametros,
			beforeSend: function(objeto) {
				$("#mensaje_resultados").html("Cargando...");
			},
			success: function(datos) {
				$('#delete_covenant').attr("disabled", false);
				$('#deleteCovenant').modal('hide');
	...
	$('#deleteCovenant').on('show.bs.modal', function (event) {
		var button = $(event.relatedTarget); // Button that triggered the modal
	...
	$("#add_covenant_doc").submit(function( event ) {
		...
		$.ajax({
			...
			contentType: false,
			beforeSend: function(objeto) {
				$("#resultados_modal_agregar[1]").html("Cargando...");
			},
			success: function(datos) {
				$('#add_covenant_doc').attr("disabled", false);
				$("#resultados_modal_agregar[1]").html(datos);
				$('#addCovenantDoc').modal('hide');
				load();
			},
			error: function() {
				$("#resultados_modal_agregar[1]").html("Error en el registro");
			}
		});
	...
	$('#addCovenantDoc').on('show.bs.modal', function (event) {
		...
		var id = button.data('id');
		var modal = $(this);
		modal.find('.modal-body #vg_id_convenio').val(id);
		modal.find('.modal-body #vg_nombre').val("");
		modal.find('.modal-body #vg_descripcion').val("");

		$("#resultados_modal_agregar[1]").html("");
	})
