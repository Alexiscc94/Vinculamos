<?php
	...
	if($id_evaluation == "") {
		$evaluations = getEvaluationsByInitiative($id);
	} else {
		$evaluations = getEvaluationByInitiativeIdEvaluation($id, $id_evaluation);
	}
	...
	for ($i=0; $i < sizeof($evaluatorTypes); $i++) {
		if($evaluatorTypes[$i]["nombre"] == $evaluations[0]["tipo_evaluacion"]) {
...
<div class="wrapper">
	<?php
		...
		include_once("modal_add_evaluation.php");
					...
					<div class="col-xs-12">
						<div class="box">
							<div class="box-header">
	    						<h3 class="box-title"><?php echo ($datas[0]["nombre"] == "" ? "Editar iniciativa" : $datas[0]["nombre"]);?> - Evaluaciones</h3>
							</div>
							<!-- /.box-header -->
								 	...
								 	if(sizeof($evaluatorTypes) == 0) { ?>
										...
										<div class="modal-footer">
				      					...
				      					if(canUpdateInitiatives()) { ?>
													<a href="edit_initiative_plan_step1.php?data=<?php echo$data;?>" class="btn btn-orange" data-dismiss="modal"> <i class="fa fa-table"></i> Editar iniciativa</a>
											<?php
												} ?>
										</div>
								<?php
...
<script>
	...
	function load() {
		...
		loadEvaluation($("#vg_tipo_evaluador").val())
		//$("#loader").html("");
		//$("#resultados_esperados").html("");
	}

	function loadEvaluation(tipo) {
		var parametros = {
	...
	$('#addEvaluation').on('show.bs.modal', function (event) {
	...
	$("#add_evaluation").submit(function( event ) {
		$('#add_evaluation').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			url: "./ajax_add_evaluation.php",
			data: parametros,
