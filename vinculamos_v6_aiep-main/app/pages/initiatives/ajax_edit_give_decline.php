<?php
	...
	if(false) {
		...
		echo "<br>vg_observaciones: " . $_REQUEST['vg_observaciones'];
		//return;
	}
	...
	if( isset($_REQUEST['vg_id']) && isset($_REQUEST['vg_usuario']) ) {
		...
		$result = editInitiativePlanStatus(noeliaDecode($_REQUEST['vg_id']),
			"Rechazada", noeliaDecode($_REQUEST['vg_usuario']));
		...
		if($result != null) {
			if(true) {
				...
				$asunto = "Iniciativa rechazada en plataforma Vinculamos";
				$mensaje = "";
				$mensaje = "Estimado/a,<br>";
				$mensaje .= "Su iniciativa <strong>#$idIniciativa \"$nombreIniciativa\"</strong> ha sido RECHAZADA en la plataforma Vinculamos con las siguientes observaciones:. ";
				$mensaje .= "<blockquote> <p>" . $_REQUEST['vg_observaciones'] . "</p> </blockquote>";
