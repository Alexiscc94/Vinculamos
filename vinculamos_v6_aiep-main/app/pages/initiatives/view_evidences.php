...
<div class="wrapper">
	<?php
		...
		include_once("modal_edit_evidence.php");
...
<script>
	...
	function loadEvidences() {
		...
		var url = "./ajax_view_evidences.php";
		$.ajax({
			type: "POST",
      url: url,
      data:  parametros,
	...
	$('#editEvidence').on('show.bs.modal', function (event) {
		var button = $(event.relatedTarget); // Button that triggered the modal
		...
		$("#resultados_modal_editar_evidencia").html("");
	})

	$("#edit_evidence").submit(function( event ) {
		$('#edit_evidence').attr("disabled", true);
		var parametros = $(this).serialize();
		$.ajax({
			type: "POST",
			url: "./ajax_edit_evidence.php",
			data: parametros,
			beforeSend: function(objeto) {
				$("#resultados_modal_editar_evidencia").html("Cargando...");
			},
			success: function(datos) {
				$("#resultados_modal_editar_evidencia").html(datos);
				loadEvidences();
			},
			error: function() {
				$("#resultados_modal_editar_evidencia").html("Error en el registro");
			}
		});
	...
	$('#deleteEvidence').on('show.bs.modal', function (event) {
		...
		$("#resultados_modal_eliminar_evidencia").html("");
	})
