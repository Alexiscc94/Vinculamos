<?php
	...
	/*
		...
		FROM "viga_participacion_real" WHERE 1
	*/

	function addRealParticipation($id_iniciativa = null, $tipo = null, $tipo2 = null, $publico_general = null,
		...
		$aplica_etnia = null, $etnia_mapuche = null, $etnia_otro = null,
		$autor = null) {
		include("db_config.php");

		$db->insert("viga_participacion_real",
			[
				...
				"aplica_etnia" => $aplica_etnia,
				"etnia_mapuche" => $etnia_mapuche,
	...
	function editRealParticipation($id = null, $id_iniciativa = null, $tipo = null, $tipo2 = null, $publico_general = null,
		...
		$db->update("viga_participacion_real",
			[
				"tipo" => $tipo,
				"tipo2" => $tipo2,
				"publico_general" => $publico_general,
		...
		if($verificator == 27) {
			include_once("medoo_logs.php");
			logAction($db, $autor, "participacion_real", $id, "Nuevo registro con valores {publico_general => $publico_general, aplica_sexo => $aplica_sexo, sexo_masculino => $sexo_masculino, sexo_femenino => $sexo_femenino, sexo_otro => $sexo_otro, aplica_edad => $aplica_edad, edad_ninos => $edad_ninos, edad_jovenes => $edad_jovenes, edad_adultos => $edad_adultos, edad_adultos_mayores => $edad_adultos_mayores, aplica_procedencia => $aplica_procedencia, procedencia_rural => $procedencia_rural, procedencia_urbano => $procedencia_urbano, aplica_vulnerabilidad => $aplica_vulnerabilidad, vulnerabilidad_pueblo => $vulnerabilidad_pueblo, vulnerabilidad_discapacidad => $vulnerabilidad_discapacidad, vulnerabilidad_pobreza => $vulnerabilidad_pobreza, aplica_nacionalidad => $aplica_nacionalidad, nacionalidad_chileno => $nacionalidad_chileno, nacionalidad_migrante => $nacionalidad_migrante, nacionalidad_pueblo => $nacionalidad_pueblo, autor => $autor}");
			return $datas;
		}return null;
	}

	function deleteRealParticipation($id = null, $id_iniciativa = null, $autor = null) {
		include("db_config.php");
		...
		if($verificator == 1) {
			include_once("medoo_logs.php");
			logAction($db, $autor, "participacion_real", $id, "Eliminación de registro con valores {id => $id, id_iniciativa => $id_iniciativa, visible => -1}");
			return $datas;
		}return null;
	}

	function getVisibleRealParticipationByInitiative($id_iniciativa = null) {
		include("db_config.php");

		$datas = $db->select("viga_participacion_real",
			[
	...
	function sumRealParticipationByInitiativeColumn($id_iniciativa = null, $column = null) {
		include("db_config.php");

		$datas = $db->sum("viga_participacion_real", $column,
			[
				"visible" => "1",
				"id_iniciativa" => $id_iniciativa
			]
	...
	function sumRealParticipationByInitiativeColumnPublicType($id_iniciativa = null, $column = null, $type = null) {
		include("db_config.php");
	...
	function sumGeneralRealParticipationByInitiativeType2($id_iniciativa = null, $type2 = null) {
		include("db_config.php");
