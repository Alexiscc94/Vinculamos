<?php
	...
	function getVisibleUsersByRole($role = null) {
		include("db_config.php");
	...
	function validateUser($username = null, $contrasenia = null) {
		...
		include_once("medoo_logs.php");
		if(sizeof($datas) > 0) {
			logAction($db, $username, "usuario", 0, "Inicio de sesión exitoso {nombre_usuario => $username}");
		} else {
			logAction($db, $username, "usuario", 0, "Inicio de sesión fallido {nombre_usuario => $username}");
		}
		return $datas;
	}
