<?php
	...
	*/
	...
	function getVisibleMeasuresByInitiative($id_initiative) {
		...
		$initiative = $db->select("viga_iniciativas_plan",
			[
				"id", "orden", "nombre", "descripcion", "objetivo", "visible", "institucion", "autor"
			],
			[
				"visible" => "1",
				"id" => $id_initiative
			...
			for($j=0 ; $j<sizeof($measures) ; $j++) {
		      ...
		      [
				...
				for($x=0 ; $x<sizeof($concepts) ; $x++) {
					...
					if (strpos(strtolower($initiative[0]["nombre"]), $conceptoMinuscula) !== false ||
						strpos(strtolower($initiative[0]["objetivo"]), $conceptoMinuscula) !== false ||
